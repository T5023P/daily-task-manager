...
const getTaskListSettingsDocRef = (userOrUid) => {
  const { uid, isAdmin } = resolvePath(userOrUid);
  if (isAdmin) {
    return doc(db, 'taskListSettings', 'default');
  }
  return doc(db, 'users', requireUid(uid), 'taskListSettings', 'default');
};
...
export const getTaskLists = (userOrUid, defaults, onUpdate) => {
  const { uid, isAdmin } = resolvePath(userOrUid);
  if ((!isAdmin && !uid) || !Array.isArray(defaults)) {
    onUpdate(defaults || []);
    return () => {};
  }

  try {
    const docRef = getTaskListSettingsDocRef(userOrUid);
    return onSnapshot(
      docRef,
      (docSnap) => {
        const savedLists = docSnap.exists() ? docSnap.data().lists || [] : [];
        const savedByKey = new Map(savedLists.map((list) => [list.key, list]));
        const mergedDefaults = defaults.map((list) => ({
          ...list,
          ...(savedByKey.get(list.key) || {}),
        }));
        const defaultKeys = new Set(defaults.map((list) => list.key));
        const customLists = savedLists.filter((list) => list?.key && !defaultKeys.has(list.key));
        onUpdate([...mergedDefaults, ...customLists]);
      },
      (error) => {
        console.error('Firestore onSnapshot error (TaskLists):', error);
        onUpdate(defaults);
      }
    );
  } catch (err) {
    console.error('Error setting up task list settings listener:', err);
    onUpdate(defaults);
    return () => {};
  }
};

export const saveTaskLists = async (userOrUid, lists) => {
  try {
    const docRef = getTaskListSettingsDocRef(userOrUid);
    await setDoc(docRef, {
      lists,
    ...
    console.error('Error saving task lists:', error);
    throw error;
  }
};
