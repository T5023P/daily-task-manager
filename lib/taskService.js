...
const getTaskDocRef = (uid, dateStr) => {
...
const getLongTermColRef = (uid) => collection(db, 'users', requireUid(uid), 'longTermOrders');
const getLongTermDocRef = (uid, orderId) => doc(db, 'users', requireUid(uid), 'longTermOrders', orderId);
const getDailyTasksColRef = (uid) => collection(db, 'users', requireUid(uid), 'dailyTasks');

/**
 * 1. getTasksForDate
 */
export const getTasksForDate = (uid, dateStr, onUpdate) => {
  if (!uid || !dateStr) {
    onUpdate([]);
    return () => {};
  }
...
export const getLongTermOrders = (uid, currentDateStr, onUpdate) => {
  if (!uid) {
    onUpdate([]);
    return () => {};
  }
...
export const getDaysWithTasks = async (uid, yearMonthStr) => {
  if (!uid) return [];
